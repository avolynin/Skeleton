...
def draw_skeleton_per_person(img, limbs, all_keypoints, all_scores, confs, keypoint_threshold=2, conf_threshold=0.9):
    # initialize a set of colors from the rainbow spectrum
    cmap = plt.get_cmap('rainbow')
    # create a copy of the image
    img_copy = img.copy()
...
def getImageWithBones(image):
    ...
    keypoints_img = draw_keypoints_per_person(image, output["keypoints"],
    ...
    skeletal_img = draw_skeleton_per_person(keypoints_img, limbs,
                                            output["keypoints"],
                                            output["keypoints_scores"], 
                                            output["scores"],
                                            keypoint_threshold=2)
